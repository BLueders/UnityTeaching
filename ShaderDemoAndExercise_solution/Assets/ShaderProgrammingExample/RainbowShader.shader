Shader "Exercises/RainbowShader"
{
	...
	SubShader
	{
		...
		Pass // A shader can have multible passes. One pass = one time render everything.
		{
			...
			vertexOutput vertexShader (vertexInput vInput)
			{
				...
				vOutput.worldCoord = vOutput.position;
			...
			{
				fixed4 col = fixed4(abs(sin(vOutput.worldCoord.y)), abs(sin(vOutput.worldCoord.y * 3.3)), abs(sin(vOutput.worldCoord.y * 7.1)), 1);
				return col;
			}
